...
function renderButtons() {
  ...
  document.querySelectorAll('.btn').forEach((button) => {
    const ingredient = button.classList[1].split('-')[1]; // Extract ingredient name from class
    if (state[ingredient]) {
...
function renderPrice() {
  ...
  // Show "No extra ingredients" if only base pizza
  if (totalPrice === basePrice) {
    pricePanel.querySelector('ul').innerHTML = `<li>No extra ingredients</li>`;
  } else {
    pricePanel.querySelector('ul').innerHTML = priceList;
  }
